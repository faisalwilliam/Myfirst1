...
private:
    ...
    void handleLogin() {
        ...
        int pin;
        int försök = 0;
        bool ärAutentiserad = false;
        // Tillåt upp till 3 inloggningsförsök
        while (!ärAutentiserad && försök < 3) {
...
public:
    ...
    Bank() : currentUserIndex(-1) {
        ...
        users.push_back(User("aisha", 2345, { Account("\tLönekonto", 12000.00) }));
        users.push_back(User("safa", 2012, { Account("\tLöneKonto",13000.00) }));
        users.push_back(User("anfal", 2014, { Account("\tLöneKonto",14000.00) }));
        users.push_back(User("amir", 2023, { Account("\tLöneKonto",15000.00) }));
